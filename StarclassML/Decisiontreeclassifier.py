...
if __name__ == '__main__':
    ...
    pca.fit(X_train)
    X_pca = pca.transform(X_train)      #Ci devo mettere xtrain o x?
    ...
    y_pred = clf.predict(X_test)
    y_pred_tr = clf.predict(X_train)    #questi sono inutili


    #crossvalidation
    scores = cross_val_score(clf, X, y, cv=5)
    print('Cross validation Accuracy: %0.4f (+/- %0.4f)' % (scores.mean(), scores.std() * 2))
    #Randomforest ottimo per feature selection
    #Prima di fare la pca si standardizzano i dati
    #crossvalidation si usa su tutto il dataset, ma prima si tunano gli hyperparametri(cambia su overleaf)
    #ha senso fare la pca sui decision tree se poi si usa l'entropia come criterio perchè sono in qualche modo legati